                ...
                if args.model == 'resnet':
                    if args.enable_lat:
                        model.z0_reg.data = args.alpha * model.z0_reg.data + \
                                          model.z0.grad / torch.norm(torch.norm(torch.norm(model.z0.grad, p = 1,dim = 2),p = 1,dim = 2),p = 1,dim = 1).view(args.batchsize,1,1,1).repeat(1,16,32,32)
                        ...
                        for i in range(len(net)):
                            for j in range(len(net[i])):
                                net[i][j].z1_reg.data = args.alpha * net[i][j].z1_reg.data + \
                                          net[i][j].z1.grad / torch.norm(torch.norm(torch.norm(net[i][j].z1.grad, p = 1,dim = 2),p = 1,dim = 2),p = 1,dim = 1).view(args.batchsize,1,1,1).repeat(1,net[i][j].planes,net[i][j].init_imgSize,net[i][j].init_imgSize) 
                                net[i][j].z2_reg.data = args.alpha * net[i][j].z2_reg.data + \
                                          net[i][j].z2.grad / torch.norm(torch.norm(torch.norm(net[i][j].z2.grad, p = 1,dim = 2),p = 1,dim = 2),p = 1,dim = 1).view(args.batchsize,1,1,1).repeat(1,net[i][j].planes,net[i][j].imgSize,net[i][j].imgSize) 
                                net[i][j].z3_reg.data = args.alpha * net[i][j].z3_reg.data + \
                                          net[i][j].z3.grad / torch.norm(torch.norm(torch.norm(net[i][j].z3.grad, p = 1,dim = 2),p = 1,dim = 2),p = 1,dim = 1).view(args.batchsize,1,1,1).repeat(1,net[i][j].planes * net[i][j].expansion,net[i][j].imgSize,net[i][j].imgSize) 
                                '''
                                ...
                                '''
                        model.x_reg.data = args.alpha * model.x_reg.data + \
                                          model.input.grad / torch.norm(torch.norm(torch.norm(model.input.grad, p = 1,dim = 2),p = 1,dim = 2),p = 1,dim = 1).view(args.batchsize,1,1,1).repeat(1,3,32,32)
