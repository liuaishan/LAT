...
class Attack():
    ...
    def return_trainloader(self):
        if self.dataset == 'cifar10':
            train_dataset = datasets.CIFAR10(root=self.dataroot,train=True, transform=transforms.ToTensor())
...
def display(test_data_cln, test_data_adv, test_label, test_label_adv):
    ...
    label_adv = test_label_adv.cpu()
    disp_batch = 10
    for a in range(2*disp_batch,3*disp_batch): 
        plt.figure()
        plt.subplot(121)
...
if __name__ == "__main__":
    ...
    # if not mnist then normalize eps from [0,255] to [0,1]
    ...
    #imgpath = args.savepath + "img_eps{}/".format(eps)
    #save_img(imgpath, test_data_cln, test_data_adv, test_label, test_label_adv)
