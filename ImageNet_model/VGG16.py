...
class VGG16(nn.Module):
    def __init__(self, enable_lat, epsilon, pro_num, batch_size = 128, num_classes=200, if_dropout = False):
        ...
        self.classifier_with_dropout = nn.Sequential(nn.Linear(512 * 7 * 7, 4096),
        ...
        self.classifier_without_dropout = nn.Sequential(nn.Linear(512 * 7 * 7, 4096),
                                                     nn.ReLU(inplace = True),
                                                     nn.Linear(4096, 4096),
                                                     nn.ReLU(inplace = True),
                                                     nn.Linear(4096, num_classes),
    ...
    def forward(self, x):
        ...
        if(self.if_dropout):
            out = self.classifier_with_dropout(out)
        else:
            out = self.classifier_without_dropout(out)
        return out
