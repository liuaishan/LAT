...
class ResNet18(nn.Module):
    def __init__(self, enable_lat, epsilon, pro_num, batch_size = 64, num_classes=10, if_dropout = False):
        self.in_planes = 64
        block = BasicBlock
        num_blocks = [2,2,2,2]
        if torch.cuda.device_count() > 1:
            batch_size = batch_size // torch.cuda.device_count()
        print('| Resnet 18 for CIFAR10')
        super(ResNet18, self).__init__()
        self.register_buffer('x_reg', torch.zeros([batch_size, 3, 32, 32]))
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.register_buffer('z0_reg', torch.zeros([batch_size, 64, 32, 32]))
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU()
        self.layer1 = self._make_layer(enable_lat, epsilon, pro_num, batch_size,block, 64, num_blocks[0], stride=1, imgSize=32)
        self.layer2 = self._make_layer(enable_lat, epsilon, pro_num, batch_size,block, 128, num_blocks[1], stride=2, imgSize=16)
        self.layer3 = self._make_layer(enable_lat, epsilon, pro_num, batch_size,block, 256, num_blocks[2], stride=2, imgSize=8)
        self.layer4 = self._make_layer(enable_lat, epsilon, pro_num, batch_size,block, 512, num_blocks[3], stride=2, imgSize=4)
    ...
    def adjust_reg(self, new_batchsize):
        self.x_reg.data = torch.zeros([new_batchsize, 3, 32, 32])
        self.z0_reg.data = torch.zeros([new_batchsize, 64, 32, 32])
        num_blocks=[2,2,2,2]
        for i in range(0,4):
            for j in range(0,num_blocks[i]):
                for k in range(0,2):
